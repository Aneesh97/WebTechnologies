...
async function create() {
    try {
        ...
        await db.run("create table content (contentid primary key, url, prompt, image)");

        //Create table for journal entries
        await db.run(
          "create table journal (journalid primary key, id, prompt, entry, timestamp, contentid, " +
                "foreign key(id) references userCredentials(id), " +
